...
var game = {
  initialize: function () {
    ...
    game.$modal.dialog({
      ...
      buttons: {
        ...
        "2 Player": function () {
          $(this).dialog("close");
          game.setHiddenPhrase(
            prompt(
              "What word will the hangman hide?",
    ...
    game.$buttons.on("click", ".guess", function () {
      game.setCurrentGuess(
        prompt("What letter would you like to guess?", "Type letter here")
      );
  ...
  renderEmptyBoxes: function (phrase) {
    ...
    for (var i = 0; i < phrase.length; i++) {
      phrase[i] === " "
        ? game.$letters.append($('<div class="space"></div>'))
        : game.$letters.append($('<div class="letter"></div>'));
    }
  ...
  setCurrentGuess: function (guess) {
    ...
    if (game.guesses[guess]) {
      ...
      game.setCurrentGuess(
        prompt("One letter at a time, please. Guess again!", "Type letter here")
      );
    } else {
  ...
  gameOver: function (winOrLose) {
    ...
    if (winOrLose === "win") {
      ...
      game.$guesses.append(
        $('<span class="loser">YOU LOSE. TRY AGAIN!</span>')
      );
    }
    $(".guess").remove();
  },
